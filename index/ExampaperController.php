...
class ExampaperController extends CommonController {
    ...
    public function value(){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM exampaper ".$where." group by ".$xColumnName." LIMIT 10";
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM exampaper ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM exampaper ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM exampaper ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function group($columnName){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM exampaper ".$where." GROUP BY ".$columnName." ORDER BY $columnName asc LIMIT 10";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
    ...
    public function compose(){
        ...
        # 单选题
        if ($radioNum > 0) {
            $radioSizeQuery = "SELECT COUNT(*) AS count FROM examquestionbank WHERE type = 0";
            ...
            if($radiocount->num_rows < 0 || $radiocount->fetch_assoc()['count']<$radioNum) {
                ...
                // 随机选择指定数量的单选题
                $radioListQuery = "SELECT * FROM examquestionbank WHERE type = 0 ORDER BY RAND() LIMIT $radioNum";
        ...
        if ($multipleChoiceNum > 0) {
            $multipleChoiceNumSizeQuery = "SELECT COUNT(*) AS count FROM examquestionbank WHERE type = 1";
            ...
            if($multipleChoiceNumcount->num_rows < 0 || $multipleChoiceNumcount->fetch_assoc()['count']<$multipleChoiceNum) {
                ...
                // 随机选择指定数量的多选题
                $multipleChoiceNumListQuery = "SELECT * FROM examquestionbank WHERE type = 1 ORDER BY RAND() LIMIT $multipleChoiceNum";
        ...
        # 判断题
        if ($determineNum > 0) {
            $determineNumSizeQuery = "SELECT COUNT(*) AS count FROM examquestionbank WHERE type = 2";
            ...
            if($determineNumcount->num_rows < 0 || $determineNumcount->fetch_assoc()['count']<$determineNum) {
                ...
                // 随机选择指定数量的判断题
                $determineNumListQuery = "SELECT * FROM examquestionbank WHERE type = 2 ORDER BY RAND() LIMIT $determineNum";
         ...
         # 填空题
         if ($fillNum > 0) {
            $fillNumSizeQuery = "SELECT COUNT(*) AS count FROM examquestionbank WHERE type = 3";
            ...
            if($fillNumcount->num_rows < 0 || $fillNumcount->fetch_assoc()['count']<$fillNum) {
                ...
                // 随机选择指定数量的填空题
                $fillNumListQuery = "SELECT * FROM examquestionbank WHERE type = 3 ORDER BY RAND() LIMIT $fillNum";
        ...
        if ($subjectivityNum > 0) {
            $subjectivityNumSizeQuery = "SELECT COUNT(*) AS count FROM examquestionbank WHERE type = 4";
            ...
            if($subjectivityNumcount->num_rows < 0 || $subjectivityNumcount->fetch_assoc()['count']<$subjectivityNum) {
                ...
                // 随机选择指定数量的填空题
                $subjectivityNumListQuery = "SELECT * FROM examquestionbank WHERE type = 4 ORDER BY RAND() LIMIT $subjectivityNum";
        ...
        if (!empty($questionList)) {
            ...
            foreach ($questionList as $q) {
                ...
                // $mysql = "INSERT INTO `examquestion` (`id`,`paperid`,`papername`,`questionname`,`options`,`score`,`answer`,`analysis`,`type`,`sequence`) VALUES (1698031523,1,十万个为什么,'下面动物不属于昆虫的是（）。','[{"text":"A.苍蝇","code":"A"},{"text":"B.蜜蜂","code":"B"},{"text":"C.蜂鸟","code":"C"}]',20,'C','蜂鸟',0,1)";
                $result = table_sql($sql);
                // print("sql:".$sql."\n");
                $seq++;
            }
