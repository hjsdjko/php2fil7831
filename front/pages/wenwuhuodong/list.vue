		...
		<view :style='{"minHeight":"100vh","padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"block","width":"100%","position":"relative","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"padding":"20rpx 24rpx 20rpx 0","margin":"0 0 0 auto","background":"#fff","display":"flex","width":"calc(100% - 0px)","height":"auto","order":"0"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #FC6103","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#f7f7f7","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.huodongbiaoti" type="text" placeholder="活动标题" ></input>
				</view>
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing07" :style='{"color":"#FC6103","left":"0px","textAlign":"center","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"2rpx solid #FC6103","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#f7f7f7","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.huodongdidian" type="text" placeholder="活动地点" ></input>
				</view>
			...
			<view :style='{"margin":"0px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto","order":"3"}'>
				<scroll-view scroll-y="true" class="category-two" :style='{"width":"128rpx","background":"#fff","height":"auto"}'>
					<view :class='categoryName === item.huodongfenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.huodongfenlei)">{{item.huodongfenlei}}</view>
				</scroll-view>
					...
					<view id="waterfall-left-column-list-6" class="waterfall-column-list-6" :style='{"margin":"0 12rpx 0 0","flex":"1","flexDirection":"column","display":"flex"}'>
						<view @tap="onDetailTap(product)" :style='{"boxShadow":"none","margin":"0 0 20rpx 0","overflow":"hidden","position":"relative","borderRadius":"12rpx"}' v-for="product in leftListList6" :key="product.id" class="left-content">
							...
							<view :style='{"width":"100%","position":"absolute","left":"0","background":"rgba(0,0,0,.3)","bottom":"44rpx"}'>
								<view :style='{"padding":"0 20rpx","lineHeight":"2","fontSize":"30rpx","color":"#fff","textAlign":"center"}' class="title ">{{product.huodongbiaoti}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenwuhuodong','修改')) || (!userid && isAuthFront('wenwuhuodong','修改'))" @click.stop="onUpdateTap(product)">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenwuhuodong','删除')) || (!userid && isAuthFront('wenwuhuodong','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('huodongfenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('huodongfenlei', {page:1,limit:100});
            }
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`wenwuhuodong`, params);
                } else {
                    res = await this.$api.list(`wenwuhuodong`, params);
                }
				// 如果是第一页数据置空
				this.list = [];
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wenwuhuodong', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`wenwuhuodong`, searchForm);
                } else {
                    res = await this.$api.list(`wenwuhuodong`, searchForm);
                }
				// 如果是第一页数据置空
