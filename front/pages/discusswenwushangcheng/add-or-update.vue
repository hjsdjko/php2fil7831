	...
	<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"#fff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0","background":"#f7f7f7","display":"flex","width":"100%","borderStyle":"solid","justifyContent":"space-around","height":"auto"}' class="">
				<view :style='{"width":"140rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","textAlign":"right","fontWeight":"500"}' class="title">评论内容</view>
                <xia-editor ref="editor" :style='{"minHeight":"300rpx","border":"2rpx solid #efefef","width":"calc(100% - 200rpx)","padding":"20rpx","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusswenwushangcheng`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusswenwushangcheng`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusswenwushangcheng`, this.ruleForm);
						}else{
							await this.$api.add(`discusswenwushangcheng`, this.ruleForm);
						}
						uni.setStorageSync('discusswenwushangchengCleanType',true);
						var res1 = await this.$api.info('wenwushangcheng',this.ruleForm.refid)
						res1.data.discussnum++
						await this.$api.update('wenwushangcheng',res1.data)
