<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","fontSize":"14px","color":"#666","background":"none","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0px 0 5px","color":"#666","flexWrap":"wrap","textAlign":"center","display":"flex","width":"100%","fontSize":"inherit","justifyContent":"center"}'>
					<el-button class="add" v-if="isAuth('wenwuhuodong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('wenwuhuodong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"10px","borderColor":"rgba(252,234,238,1)","borderRadius":"10px","background":"rgba(255,255,255,1)","borderWidth":"6px 3px 2px 3px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('wenwuhuodong','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('wenwuhuodong','预约')" @click="huodongyuyueCrossAddOrUpdateHandler(scope.row,'cross','','[1]','已预约')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								预约
							</el-button>
							<el-button class="edit" v-if=" isAuth('wenwuhuodong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('wenwuhuodong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<huodongyuyue-cross-add-or-update v-if="huodongyuyueCrossAddOrUpdateFlag" :parent="this" ref="huodongyuyueCrossaddOrUpdate"></huodongyuyue-cross-add-or-update>
...
import huodongyuyueCrossAddOrUpdate from "../huodongyuyue/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				huodongyuyueCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			huodongyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.huodongyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','wenwuhuodong');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.huodongyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.huodongyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	wenwuhuodongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'wenwuhuodong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
