		...
		<view :style='{"minHeight":"100vh","padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"block","width":"100%","position":"relative","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"padding":"20rpx 24rpx 20rpx 0","margin":"0 0 0 auto","background":"#fff","display":"flex","width":"calc(100% - 0px)","height":"auto","order":"0"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #FC6103","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#f7f7f7","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.wenwuleibie" type="text" placeholder="文物类别" ></input>
				</view>
					...
					<view id="waterfall-left-column-list-6" class="waterfall-column-list-6" :style='{"margin":"0 12rpx 0 0","flex":"1","flexDirection":"column","display":"flex"}'>
						<view @tap="onDetailTap(product)" :style='{"boxShadow":"none","margin":"0 0 20rpx 0","overflow":"hidden","position":"relative","borderRadius":"12rpx"}' v-for="product in leftListList6" :key="product.id" class="left-content">
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenwuleibie','修改')) || (!userid && isAuthFront('wenwuleibie','修改'))" @click.stop="onUpdateTap(product)">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenwuleibie','删除')) || (!userid && isAuthFront('wenwuleibie','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.wenwuleibie){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`wenwuleibie`, params);
                } else {
                    res = await this.$api.list(`wenwuleibie`, params);
                }
				// 如果是第一页数据置空
				this.list = [];
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wenwuleibie', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.wenwuleibie){
					searchForm['wenwuleibie'] = '%' + this.searchForm.wenwuleibie + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`wenwuleibie`, searchForm);
                } else {
                    res = await this.$api.list(`wenwuleibie`, searchForm);
                }
				// 如果是第一页数据置空
