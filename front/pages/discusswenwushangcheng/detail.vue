...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusswenwushangchengCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusswenwushangchengCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusswenwushangcheng', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discusswenwushangcheng');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusswenwushangcheng');
			...
			async init(){
				...
				let res = await this.$api.info('discusswenwushangcheng', this.id);
				this.detail = res.data;
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusswenwushangcheng', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusswenwushangcheng/add-or-update?refid=${this.id}`)
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusswenwushangcheng',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusswenwushangcheng',that.detail)
							setTimeout(()=>{
