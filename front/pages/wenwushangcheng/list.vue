		...
		<view :style='{"minHeight":"100vh","padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"block","width":"100%","position":"relative","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"padding":"20rpx 24rpx 20rpx 0","margin":"0 0 0 auto","background":"#fff","display":"flex","width":"calc(100% - 0px)","height":"auto","order":"0"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"2rpx solid #FC6103","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"#f7f7f7","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.shangpinmingcheng" type="text" placeholder="商品名称" ></input>
				</view>
			...
			<view :style='{"padding":"12rpx 40rpx","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","order":"1"}'>
				<view @click="sortClick('price')" :style='{"border":"0","padding":"0 12rpx","margin":"0 8rpx 0 0","outline":"0","borderRadius":"8rpx","background":"#FC6103","display":"flex"}'>
					<text :style='{"color":"#fff","lineHeight":"48rpx","fontSize":"24rpx"}'>价格</text>
					<text v-if="listSort!='price'" class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 0 0 4rpx","lineHeight":"48rpx","fontSize":"24rpx","color":"#fff"}'></text>
					<text v-else-if="listSort=='price'&&listOrder=='asc'" class="icon iconfont icon-jiantou06" :style='{"margin":"0 0 0 4rpx","lineHeight":"48rpx","fontSize":"24rpx","color":"#fff"}'></text>
					<text v-else-if="listSort=='price'&&listOrder=='desc'" class="icon iconfont icon-jiantou07" :style='{"margin":"0 0 0 4rpx","lineHeight":"48rpx","fontSize":"24rpx","color":"#fff"}'></text>
				</view>
					...
					<view id="waterfall-left-column-list-6" class="waterfall-column-list-6" :style='{"margin":"0 12rpx 0 0","flex":"1","flexDirection":"column","display":"flex"}'>
						<view @tap="onDetailTap(product)" :style='{"boxShadow":"none","margin":"0 0 20rpx 0","overflow":"hidden","position":"relative","borderRadius":"12rpx"}' v-for="product in leftListList6" :key="product.id" class="left-content">
							...
							<view :style='{"width":"100%","position":"absolute","left":"0","background":"rgba(0,0,0,.3)","bottom":"44rpx"}'>
								<view :style='{"padding":"0 20rpx","lineHeight":"2","fontSize":"30rpx","color":"#fff","textAlign":"center"}' class="title ">{{product.shangpinmingcheng}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenwushangcheng','修改')) || (!userid && isAuthFront('wenwushangcheng','修改'))" @click.stop="onUpdateTap(product)">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('wenwushangcheng','删除')) || (!userid && isAuthFront('wenwushangcheng','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`wenwushangcheng`, params);
                } else {
                    res = await this.$api.list(`wenwushangcheng`, params);
                }
				// 如果是第一页数据置空
				this.list = [];
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('wenwushangcheng', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`wenwushangcheng`, searchForm);
                } else {
                    res = await this.$api.list(`wenwushangcheng`, searchForm);
                }
				// 如果是第一页数据置空
